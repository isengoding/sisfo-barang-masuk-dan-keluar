            ...
            <div class="row g-2 align-items-center">
                ...
                <div class="col-auto ms-auto d-print-none">
                    <div class="btn-list">
                        ...
                        <a href="{{ route('barang-masuk.edit', $barangMasuk) }}"
                            class="btn btn-primary d-none d-sm-inline-block">
                            <i class="ti ti-edit icon me-2"></i>
                            Edit
                        </a>
                        <a href="#" class="btn btn-danger d-none d-sm-inline-block"
                            onclick="handleDelete(`{{ route('barang-masuk.destroy', $barangMasuk->id) }}`)">
                            <i class="ti ti-trash icon me-2"></i>
                            Hapus
                        </a>
                        <a href="{{ route('barang-masuk.create') }}" class="btn btn-primary d-sm-none btn-icon"
                        ...
                        <div class="card-body">
                            <div class="row">
                                ...
                                <div class="col-6 text-end">
                                    <p class="h3">Pemasok</p>
                                    <address>
                                        {{ $barangMasuk->pemasok->nama_pemasok }}<br>
                                        {{ $barangMasuk->pemasok->alamat }}<br>
                                        {{ $barangMasuk->pemasok->notelp }}<br>
                                        {{ $barangMasuk->pemasok->email }}
                                    </address>
                                </div>
                                <div class="col-12 my-5">
                                    <div class="fs-2">{{ $barangMasuk->no_transaksi }}</div>
                                    <div>{{ \Carbon\Carbon::parse($barangMasuk->tgl_masuk)->format('d M Y') }}</div>
                                </div>
                            </div>
                            <table class="table table-transparent table-responsive">
                                ...
                                <tbody>
                                    @foreach ($barangMasuk->barangMasukDetails as $item)
                                        <tr>
                                    ...
                                    <tr>
                                        ...
                                        <td class="font-weight-bold text-end text-nowrap">Rp.
                                            {{ number_format($barangMasuk->total_harga) }}</td>
                                    </tr>
                                </tbody>
                            </table>
